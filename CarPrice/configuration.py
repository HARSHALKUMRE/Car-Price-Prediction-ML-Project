...
from CarPrice.constant import MODEL_FILE_NAME, PREPROCSSING_OBJECT_FILE_NAME
...
class ModelRegistryConfig:
    ...
    def get_latest_model_path(self) -> str:
        ...
        try:
            ...
            logging.info("Getting the `latest model path` from the Model Registry..")
    ...
    def get_latest_transformer_path(self) -> str:
        ...
        try:
            latest_dir = self.get_latest_dir_path()
            logging.info("Getting the `latest transformer path` from the Model Registry..")
            if latest_dir is None:
                logging.exception(
                    "Even the dir doesn't exist and you are expecting a transformer, shame!")

            return os.path.join(latest_dir, self.transformer_dir, PREPROCSSING_OBJECT_FILE_NAME)
            ...
        except Exception as e:
            logging.exception(e)
            raise CarPriceException(e, sys) from e
    ...
    def get_latest_dir_path_to_save(self) -> str:
        ...
        try:
            latest_dir = self.get_latest_dir_path()
            logging.info("Configuring the dir path where the `latest artifacts` are to be saved..")
    ...
    def save_latest_transformer_at(self) -> str:
        ...
        try:
            ...
            logging.info("Configuring the path where the `latestly built Transformer` is to be stored..")
            return os.path.join(latest_dir_to_save, self.transformer_dir, PREPROCSSING_OBJECT_FILE_NAME)
            ...
        except Exception as e:
            logging.exception(e)
            raise CarPriceException(e, sys) from e
    ...
    def save_latest_model_at(self) -> str:
        ...
        try:
            latest_dir = self.get_latest_dir_path()
            logging.info("Configuring the path where the `latestly trained Model` is to be stored..")
            return os.path.join(latest_dir, self.model_dir, MODEL_FILE_NAME)
            ...
        except Exception as e:
            logging.exception(e)
