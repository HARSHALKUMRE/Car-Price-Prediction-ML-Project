...
class BasicUtils:
    ...
    def configure_float_columns(cls, df: pd.DataFrame, exclude_columns: List, desc: str) -> pd.DataFrame:
        ...
        try:
            ...
            logging.info("Typecasting done successfully!")
            return df
            ...
    ...
    def get_numerical_and_categorical_attributes(cls, df: pd.DataFrame, desc: str):
        ...
        try:
            ...
            logging.info("attributes fetched successfully!")
    ...
    def get_features_and_labels(cls, df: pd.DataFrame, target: List, desc: str) -> Tuple:
        ...
        try:
            ...
            logging.info("returning the said input features and dependent labels..")
    ...
    def save_numpy_array(cls, file_path: str, arr: np.array, desc: str):
        ...
        """
        try:
            logging.info(f'Saving the "{desc} Array" at "{file_path}"..')
            # Making sure the dir do exist
            dir = os.path.dirname(file_path)
            os.makedirs(dir, exist_ok=True)
            with open(file_path, "wb") as f:
                np.save(f, arr)
            logging.info(f'"{desc} array" saved successfully!')
            ...
        except Exception as e:
            logging.exception(e)
            raise CarPriceException(e, sys) from e
    ...
    def load_numpy_array(cls, file_path: str, desc: str):
        ...
        """
        try:
            logging.info(f'Loading the "{desc} Array" from "{file_path}"..')
            ...
            if not os.path.exists(file_path):
                ...
                logging.info(f'"{desc} Array" loaded successsfully!')
    ...
    def save_object(cls, file_path: str, obj: object, obj_desc: str) -> None:
        ...
        """
        try:
            logging.info(f'Saving the "{obj_desc}" at "{file_path}"..')
            obj_dir = os.path.dirname(file_path)
            os.makedirs(obj_dir, exist_ok=True)
            dill.dump(obj, open(file_path, 'wb'))
            logging.info(f'"{obj_desc}" saved successfully!')
            ...
        except Exception as e:
            logging.exception(e)
            raise CarPriceException(e, sys) from e
    ...
    def load_object(cls, file_path: str, obj_desc: str) -> object:
        ...
        """
        try:
            logging.info(f'loading the "{obj_desc}"..')
            if not os.path.exists(file_path):
                logging.error(
                ...
                logging.info(f'"{obj_desc}" loaded successfully!')
    ...
    def write_yaml_file(cls, file_path: str, data: dict, desc: str):
        ...
        """
        try:
            logging.info(f"readying the `{desc}` yaml file..")
